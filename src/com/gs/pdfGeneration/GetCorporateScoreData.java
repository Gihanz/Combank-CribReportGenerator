...
public class GetCorporateScoreData {
	
	public static Logger log = Logger.getLogger(GetCorporateScoreData.class);
	
	ScoreGeneralDetails scoreGeneralDetails;
	CribScore cribScore;
	List<ScoreObservations> scoreObservationsList;
	
	// Get corporate score data
	 public CorporateScoreObject getCorporateScoreDataByDetailId(Connection conn, int requestDetailId) throws SQLException, Exception {
	 	
		 CorporateScoreObject corporateObj = new CorporateScoreObject();
		 
 		CallableStatement call = conn.prepareCall("{call GET_CORP_SCORE_DATA(?)}");
		...
		corporateObj.setScoreGeneralDetails(scoreGeneralDetails);
		...
		corporateObj.setScoreObservations(scoreObservationsList);
		...
		corporateObj.setCribScore(cribScore);
